    ...
    */
    public func items<
        ...
        return { source in
            ...
            return source.subscribeProxyDataSource(ofObject: self.base, dataSource: dataSource as UITableViewDataSource, retainDataSource: true) { [weak tableViewSearch = self.base] (_: RxTableViewDataSourceProxy, event) -> Void in
                guard let tableViewSearch = tableViewSearch else {
                    return
                }
                dataSource.tableView(tableViewSearch, observedEvent: event)
            }
        }
    }
